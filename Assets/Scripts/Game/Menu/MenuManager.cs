...
public class MenuManager : MonoBehaviour
{
    ...
    public void SetUpStartMenu()
    {
        ...
        StartPages[StartPageKey] = Instantiate(StartMenuRef, RightPanel.transform);
    ...
    void SetUpParentTabs()
    {
        ...
        {
            GameObject parentButton = InstantiateButton(NetworkManager.supportedNetworkTypes[networkTypeI], ParentTabSystem.transform);
            ...
            for (int gameTypeI = 0; gameTypeI < GameMaster.possibleRules.Count; gameTypeI++)
            {
                string possibleGameType = GameMaster.possibleRules[gameTypeI].Name;
                ParentTabs[parentButton].Add(ChildTabs[possibleGameType]);
    ...
    void SetUpChildTabs() //Keeps a set of child tabs
    {
        ...
        {
            string possibleGameType = GameMaster.possibleRules[i].Name;
            ChildTabs[possibleGameType] = InstantiateButton(possibleGameType, ChildTabSystem.transform);
            ChildTabs[possibleGameType].GetComponent<TabBehavior>().Start();
            ChildTabs[possibleGameType].GetComponent<TabBehavior>().SetTextSize(24);
            ChildTabs[possibleGameType].SetActive(false);
        }
    }
    ...
    {
        if(gameMaster.networkManager.currentNetworkType == NetworkManager.Online)
        {
